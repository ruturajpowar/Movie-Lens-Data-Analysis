...
public class TopViewedReducer extends Reducer<LongWritable, Text, Text,LongWritable >{
	...
	@Override
	public void reduce(LongWritable viewCount,Iterable<Text> movieIds,Context context ) {
		if(counter < topk) {
			movieIds.forEach(movieId->{
				try {
					context.write(new Text(moviesMap.get(movieId.toString())), viewCount);
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
